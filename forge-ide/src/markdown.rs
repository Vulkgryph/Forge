...
pub fn render(ui: &mut egui::Ui, text: &str, max_run: usize) {
    ...
    while i < lines.len() {
        ...
        if is_table_start(&lines, i) {
            ...
            render_table(ui, &header, &rows, max_run);
            continue;
        }
        ...
        if let Some(rest) = line.strip_prefix("### ") {
            ui.add_space(6.0);
            ui.label(RichText::new(crate::app::soft_wrap(rest, max_run)).size(13.5).strong().color(T.heading));
            i += 1; continue;
        }
        if let Some(rest) = line.strip_prefix("## ") {
            ui.add_space(8.0);
            ui.label(RichText::new(crate::app::soft_wrap(rest, max_run)).size(15.0).strong().color(T.heading));
            i += 1; continue;
        }
        if let Some(rest) = line.strip_prefix("# ") {
            ui.add_space(10.0);
            ui.label(RichText::new(crate::app::soft_wrap(rest, max_run)).size(17.0).strong().color(T.heading));
            i += 1; continue;
        }

        // ── Bullet list ──
        if let Some(rest) = line.strip_prefix("- ").or_else(|| line.strip_prefix("* ")) {
            render_list_row(ui, "•", rest, max_run);
            i += 1; continue;
        }

        // ── Numbered list ──
        if let Some((num, rest)) = parse_numbered_prefix(line) {
            render_list_row(ui, &format!("{}.", num), rest, max_run);
            i += 1; continue;
        }
        ...
        let job = parse_inline(&crate::app::soft_wrap(&para, max_run), 12.5);
        ui.add(egui::Label::new(job).wrap());
    }
}

fn render_list_row(ui: &mut egui::Ui, marker: &str, content: &str, max_run: usize) {
    ui.horizontal_top(|ui| {
        ui.add_space(8.0);
        ui.label(RichText::new(marker).size(12.5).color(T.bullet));
        ui.add_space(4.0);
        let job = parse_inline(&crate::app::soft_wrap(content, max_run), 12.5);
        ui.add(egui::Label::new(job).wrap());
    });
}
