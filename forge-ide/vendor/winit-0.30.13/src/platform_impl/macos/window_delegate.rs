...
use objc2_app_kit::{
    ...
    NSColor, NSDraggingDestination, NSFilenamesPboardType, NSPasteboard, NSPasteboardItem,
    NSPasteboardTypeFileURL, NSRequestUserAttentionType, NSScreen, NSView, NSWindowButton,
...
/// `NSFilenamesPboardType` is the legacy pasteboard format — Finder file
/// drags still populate it for backward compatibility, but newer macOS
/// system UI (notably the floating thumbnail shown after a Cmd+Shift+4
/// screenshot) only puts a `NSPasteboardTypeFileURL` item on the
/// pasteboard, which the original upstream implementation never checked at
/// all — silently rejecting the drop outright rather than falling back.
/// This mirrors the legacy path but reads each pasteboard item's file URL
/// string and resolves it through `NSURL` (which handles the
/// percent-decoding correctly) instead of hand-parsing it.
fn file_urls_from_pasteboard(pb: &NSPasteboard) -> Vec<std::path::PathBuf> {
    use std::path::PathBuf;

    if let Some(filenames) = pb.propertyListForType(unsafe { NSFilenamesPboardType }) {
        let filenames: Retained<NSArray<NSString>> = unsafe { Retained::cast(filenames) };
        return filenames.into_iter().map(|file| PathBuf::from(file.to_string())).collect();
    }

    let Some(items) = (unsafe { pb.pasteboardItems() }) else { return Vec::new() };
    items
        .into_iter()
        .filter_map(|item: Retained<NSPasteboardItem>| unsafe {
            item.stringForType(NSPasteboardTypeFileURL)
        })
        .filter_map(|url_string| unsafe { NSURL::URLWithString(&url_string) })
...
declare_class!(
    ...
    unsafe impl NSDraggingDestination for WindowDelegate {
        ...
        fn dragging_entered(&self, sender: &NSObject) -> bool {
            ...
            let paths = file_urls_from_pasteboard(&pb);
            if paths.is_empty() {
                return false.into();
            }
