...
- **A stray arrow key no longer cancels a running turn.** A lone `ESC` is both the Escape key and the first byte of every arrow key, function key and mouse report, and nothing in the byte stream tells them apart — it is resolved on timing. The window was 25 ms, and the comment above it had already named the failure: *"too short and a slow connection turns arrow keys into Escape presses."* Since Escape now stops a turn, that was not a stray keypress — it cancelled work in progress, with nothing left behind to say why. Over SSH or on a loaded machine, 25 ms is easily exceeded.

  The window is 100 ms idle and **400 ms while a turn is running**, because the cost of guessing wrong is not the same in both directions and the evidence demanded should match it. Idle, a misread Escape does nothing at all; mid-turn it destroys work already paid for. Four hundred milliseconds is imperceptible when the agent is already working, and a sequence split by that much is not jitter — it is a broken link. Idle it stays snappy: four times the margin, still far below where a "stop" feels sluggish, and well inside the range other terminal applications use — ncurses' own default is forty times longer. The cost in the other direction is real and smaller: `ESC` followed by another byte reads as an Alt chord and is dropped, so pressing Escape and typing inside the window loses both. A lost keystroke is noticed and retyped; a cancelled turn is not recoverable.

  A tail that still arrives late is now read as the key it belongs to rather than typed. Before, `[A` turning up after a flushed `ESC` decoded as the characters `[` and `A` and landed in the prompt — so one split keypress both stopped the work and left litter behind. A sequence tail immediately after a flush is evidence the flush was wrong; the spurious Escape cannot be recalled, which is what the wider window is for, but the rest of the keypress can at least mean what it meant.

  And an interruption now names itself — "Interrupted by Escape." — because several things stop a turn and they are indistinguishable once it has stopped. Timing can never fully settle the ambiguity, so the next occurrence should at least be reportable instead of looking like the agent being killed at random.

- **A bot check now reports itself in the agent's own answer rather than as a client notice.** The agent knows this client has no browser — see the agent's `--host-can-browse` — so it no longer raises a handoff request a terminal cannot satisfy. It says what was refused and answers from what else it found. The notice remains as a fallback for a request arriving from an agent that did not get the capability.
